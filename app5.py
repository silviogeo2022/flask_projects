...
# Encoding do cliente. Em cloud, UTF8 é o mais comum.
CLIENT_ENCODING = os.getenv('CLIENT_ENCODING', 'UTF8')

def get_sqlalchemy_uri() -> str:
    """
    Preferir DATABASE_URL (Render: use o Internal Database URL).
    Fallback para PGHOST/PGPORT/PGUSER/PGPASSWORD/PGDATABASE quando em dev.
    Converte prefixos 'postgres://' para 'postgresql://' e força driver psycopg2.
    """
    ...
    if url:
        ...
        # Força driver psycopg2 explicitamente (opcional, mas deixa consistente)
...
def set_client_encoding(dbapi_connection, connection_record):
    ...
    try:
        ...
        try:
            # Em alguns servidores esse parâmetro pode não existir
