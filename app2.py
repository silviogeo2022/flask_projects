...
db_url = URL.create(
    "postgresql+psycopg2",
    username=DB_USER,
    password=DB_PASSWORD,   # seguro com caracteres especiais
    host=DB_HOST,
    port=DB_PORT,
    database=DB_NAME,
)
...
def set_client_encoding(dbapi_connection, connection_record):
    ...
    try:
        ...
        try:
            # mensagens neutras em inglês (se permitido)
...
class Solicitacao(db.Model):
    ...
    # a coluna pode não existir ainda; garantimos no startup (ver main)
    situacoes = db.Column(db.Text, nullable=True)         # CSV: "buraco,iluminacao"

# ============ Rotas ============
@app.route('/')
def index():
    # No template: <meta charset="utf-8"> e <form ... accept-charset="UTF-8">
...
def enviar_formulario():
    ...
    try:
        ...
        # deixe mais simples em produção
...
def lista():
    ...
    linhas = [f"{i.id} - {i.rua}, {i.numero} - {i.bairro} | situações: {i.situacoes or '-'}" for i in itens]
...
if __name__ == '__main__':
    # Garante que schema/tabela/coluna existem conforme seu banco
    with app.app_context():
        with db.engine.begin() as conn:
            conn.execute(text(f'CREATE SCHEMA IF NOT EXISTS "{DB_SCHEMA}"'))
            # cria a tabela mínima se não existir (nome_rua/numero/bairro)
            conn.execute(text(f'''
            ...
            '''))
            # adiciona a coluna situacoes, se estiver faltando
            conn.execute(text(f'''
